...
- **`.dem.zst` файлы**: определяются по расширению в watcher/import логике, но реальная распаковка не реализована — импорт таких файлов завершится ошибкой парсинга. Необходимо либо реализовать распаковку (потребует вернуть зависимость `zstandard`), либо явно отклонять такие файлы с понятным сообщением вместо попытки парсинга.
